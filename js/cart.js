...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const pagoCarritoButton = document.getElementById('pagoCarrito');
...
if (!inputEsquina) {
    ...
    // Validaciones antes de proceder al pago
    pagoCarritoButton.addEventListener('click', function (e) {
        e.preventDefault(); // Evitar que el formulario se envíe antes de validar

        // 1. Validar que la cantidad de productos sea mayor a 0
...
// Agregar el console.log aquí para depurar
console.log('Valor del campo Esquina:', esquinaValue); // Esto imprimirá el valor del campo Esquina en la consola
        ...
        location.href = "cart-pago.html";  // Redirigir a la página de pago
    });
});
...
document.getElementById('finalizarCompra').addEventListener('click', function (event) {
    ...
    if (metodoPagoSeleccionado === 'flexRadioDefault1') { // Si se selecciona tarjeta
...
if (!/\S+@\S+\.\S+/.test(correoElectronico)) {
    ...
    } else if (metodoPagoSeleccionado === 'flexRadioDefault2') { // Si se selecciona transferencia
        alert('Excelente solo queda un paso más. Por favor, realiza el pago mediante transferencia y envíanos el comprobante para disfrutar pronto de tu compra.');
    }
});
